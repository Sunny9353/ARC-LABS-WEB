...
function mergeAdminData(realData, fakeData) {
  return {
    orders: [...fakeData.orders, ...realData.orders],
    analyticsEvents: [...fakeData.analyticsEvents, ...realData.analyticsEvents],
    telemetryData: [...(fakeData.telemetryData || []), ...(realData.telemetryData || [])],
    leads: [...fakeData.leads, ...realData.leads],
    certificates: realData.certificates,
    eligibleRegistrations: realData.eligibleRegistrations,
  };
}

function useLocalFakeAdminData(enabled) {
  const [fakeData, setFakeData] = useState(() => (enabled ? createFakeAdminData() : emptyAdminData()));

  useEffect(() => {
    if (!enabled) {
      setFakeData(emptyAdminData());
      return undefined;
    }
    setFakeData((current) => current.orders.length ? current : createFakeAdminData());
    const timer = window.setInterval(() => {
      setFakeData((current) => appendFakeAdminTick(current));
    }, 3200);
...
function emptyAdminData() {
  return { orders: [], analyticsEvents: [], telemetryData: [], leads: [], certificates: [], eligibleRegistrations: [] };
}

const FAKE_PRODUCTS = [
  { id: "iot-essential-kit", name: "ARC LABS IoT Essential Kit", price: 14999 },
  { id: "ai-iot-lab-system", name: "AI + IoT Lab System", price: 149000 },
  { id: "robotics-starter-kit", name: "Robotics Starter Kit", price: 24999 },
  { id: "drone-lab-package", name: "Drone Lab Package", price: 189000 },
  { id: "program-curriculum", name: "Program Curriculum", price: 49999 },
];
const FAKE_NAMES = ["Aarav Sharma", "Ananya Reddy", "Sai Krishna", "Meera Nair", "Rohan Verma", "Ishaan Gupta", "Kavya Rao", "Vikram Singh", "Priya Menon", "Arjun Patel"];
const FAKE_INSTITUTES = ["Narayana Engineering College", "Hyderabad STEM Academy", "Delhi Public School", "Vignan Institute", "SRM Innovation Lab", "Bengaluru Tech School"];
const FAKE_PATHS = ["/", "/programs", "/products", "/lab-packages", "/verify", "/csr-partners", "/industrial-iot-solutions", "/checkout"];

function createFakeAdminData() {
  const orders = [];
  ...
  for (let index = 0; index < 680; index += 1) {
    const daysAgo = randomInt(0, 1320);
    const createdAt = randomPastDate(now, daysAgo);
    const product = pick(FAKE_PRODUCTS);
    const state = pick(INDIA_STATES);
    const name = pick(FAKE_NAMES);
    const paid = Math.random() > 0.18;
    orders.push(fakeOrder(index, product, name, state, createdAt, paid));
    if (Math.random() > 0.68) leads.push(fakeLead(index, createdAt, state));
  }
  for (let index = 0; index < 1700; index += 1) {
    const createdAt = randomPastDate(now, randomInt(0, 420));
    const event = fakeEvent(index, createdAt);
    analyticsEvents.push(event);
    if (Math.random() > 0.22) telemetryData.push(fakeTelemetry(index, event.region || event.state, createdAt));
  }
  return { orders, analyticsEvents, telemetryData, leads, certificates: [], eligibleRegistrations: [] };
}

function appendFakeAdminTick(current) {
  const tick = Date.now();
  const orderCount = current.orders.length;
  const eventCount = current.analyticsEvents.length;
  const product = pick(FAKE_PRODUCTS);
  const state = pick(INDIA_STATES);
  const name = pick(FAKE_NAMES);
  const nextOrders = Math.random() > 0.42
    ? [fakeOrder(orderCount + 1, product, name, state, new Date(tick).toISOString(), Math.random() > 0.16), ...current.orders]
    : current.orders;
  const nextLeads = Math.random() > 0.72
    ? [fakeLead(current.leads.length + 1, new Date(tick - randomInt(0, 900000)).toISOString(), state), ...current.leads]
    : current.leads;
  const burst = Array.from({ length: randomInt(2, 7) }, (_, index) => fakeEvent(eventCount + index + 1, new Date(tick - randomInt(0, 1800000)).toISOString()));
  const telemetryBurst = burst.map((event, index) => fakeTelemetry(eventCount + index + 1, event.region || event.state, event.createdAt));
  return {
    ...current,
    orders: nextOrders.slice(0, 900),
    analyticsEvents: [...burst, ...current.analyticsEvents].slice(0, 2200),
    telemetryData: [...telemetryBurst, ...(current.telemetryData || [])].slice(0, 2000),
    leads: nextLeads.slice(0, 420),
  };
}

function fakeOrder(index, product, name, state, createdAt, paid) {
  return {
    id: `fake-order-${index}`,
    _collection: "localFakeOrders",
    invoiceNumber: `ARC-SIM-${String(index).padStart(5, "0")}`,
    paymentId: paid ? `pay_sim_${index}_${randomInt(1000, 9999)}` : "",
    status: paid ? "paid" : pick(["failed", "cancelled", "pending"]),
    fulfillmentStatus: paid ? pick(["pending", "accepted", "packed", "shipped", "delivered"]) : "pending",
    amount: paid ? Math.round(product.price * randomFloat(0.92, 1.18)) : 0,
    product,
    ...
    customer: {
      ...
      state,
      city: state === "Telangana" ? pick(["Hyderabad", "Warangal", "Karimnagar"]) : state,
    },
    createdAt,
    paidAt: createdAt,
    isLocalFake: true,
...
function fakeLead(index, createdAt, state) {
  const type = pick(["Program Curriculum", "Lab Proposal", "CSR Partnership", "General Enquiry"]);
  return {
    id: `fake-lead-${index}`,
    _collection: "localFakeLeads",
    leadType: type,
    name: pick(FAKE_NAMES),
    email: `admin${index}@${pick(["school.edu", "college.ac.in", "stemlab.in"])}`,
    phone: `9${randomInt(100000000, 999999999)}`,
    institution: pick(FAKE_INSTITUTES),
    state,
    city: state,
    message: `${type} request for ${pick(FAKE_PRODUCTS).name}.`,
    createdAt,
    adminStatus: pick(["new", "read", "contacted", "qualified"]),
    isLocalFake: true,
  };
}

function fakeEvent(index, createdAt) {
  const path = pick(FAKE_PATHS);
  const type = pick(["page_view", "click", "section_view", "form_view", "form_submit"]);
  const state = pick(INDIA_STATES);
  const action = type === "page_view" ? "Page viewed" : pick(["View Product", "Download Brochure", "Request Lab Setup", "WhatsApp Enquiry", "Email Intent", "Submit Enquiry"]);
  return {
    id: `fake-event-${index}`,
    _collection: "localFakeAnalytics",
    type,
    path,
    pageName: pageLabel(path),
    label: action,
    actionName: action,
    sectionName: pick(["Hero", "Programs", "Products", "Lab Infrastructure", "Contact", "Pricing"]),
    location: pageLabel(path),
    href: action.includes("WhatsApp") ? "https://wa.me/919000000000" : path,
    sessionId: `sim-session-${randomInt(1, 340)}`,
    state,
    region: state,
    country: "India",
    createdAt,
...
function fakeTelemetry(index, state, createdAt) {
  const [latitude, longitude] = stateCoordinate(state);
  return {
    id: `fake-telemetry-${index}`,
    _collection: "localFakeTelemetry",
    city: state === "Telangana" ? pick(["Hyderabad", "Warangal", "Nizamabad"]) : state,
    region: state,
    country: "India",
    latitude: latitude + randomFloat(-0.38, 0.38),
    longitude: longitude + randomFloat(-0.48, 0.48),
    pagePath: pick(FAKE_PATHS),
    sessionId: `sim-session-${randomInt(1, 340)}`,
    timestamp: createdAt,
    clientCreatedAt: createdAt,
    isLocalFake: true,
...
function randomPastDate(now, daysAgo) {
  const ms = now - daysAgo * 86400000 - randomInt(0, 86400000);
...
function useAdminData(enabled) {
  ...
  const fakeData = useLocalFakeAdminData(enabled && ENABLE_LOCAL_FAKE_DATA);

  return useMemo(() => mergeAdminData(data, fakeData), [data, fakeData]);
}
